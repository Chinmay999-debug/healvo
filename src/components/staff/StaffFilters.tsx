...
const options: { key: StaffFilter; label: string }[] = [
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--color-border)] px-5 py-3">
      <div className="relative" ref={containerRef}>
        ...
        {open && (
          <div
            role="listbox"
            className="absolute left-0 z-30 mt-1.5 w-48 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-1 shadow-lg"
          >
            {options.map((option) => (
              <button
                key={option.key}
              ...
              >
                {option.label}
                <span className="text-[11.5px] font-bold text-[var(--color-muted-soft)]">
                  {counts[option.key]}
                </span>
              </button>
            ))}
